...
export function getIdiom(word: string): [string, string | undefined] | undefined {
  ...
  if (IdiomsList.includes(word)) return [word, undefined]
  if (IdiomsList.includes(simplified)) return [simplified, undefined]
  return undefined
}
...
export function getPinyin(word: string) {
  ...
  return parts.map((i) => i.replace(/^(y|j|q|x)u([a-z]*[0-9]?)$/g, '$1v$2'))
}
